...
- 当初はGoogle Tasks APIを使用していましたが、**時刻情報のサポート不足**により、Google Calendar方式に移行しました
- Google Tasks APIは日付のみをサポートし、時刻情報は破棄されます（[API仕様](https://developers.google.com/workspace/tasks/reference/rest/v1/tasks)）
- Google Calendar APIは開始時刻・終了時刻を完全にサポートしています
...
| 日付 | 日付 | **時刻を含める**をON、**終了日**をON | ✅ |
| Google Event ID | テキスト | - | ❌（自動設定） |

**重要：** 「日付」プロパティでは、必ず「時刻を含める」と「終了日」の設定をONにしてください。これにより、開始時刻（start）と終了時刻（end）の両方を管理できます。
...
### 1. 双方向同期
- **開始時刻が過去1ヶ月 ~ 未来2ヶ月の間にあるタスク**を同期対象とする
- Notion/Google Calendar双方で作成・更新・削除されたタスクを自動同期
- **高効率な処理**: 1回のループで分類・差分チェック・タイムスタンプ比較を完了
- 内容差分チェック（title/status/startTime/endTime）+ タイムスタンプ比較による競合解決（Last Write Wins）
- ID相互参照による正確な削除判定

### 2. 同期対象
- ✅ タスク名（title）
- ✅ ステータス（status）: ToDo/Done（2状態）※Google Calendar summaryプレフィックス（▶/✔）で実装
- ✅ 開始時刻（startTime）: ISO 8601形式（日時情報含む）
- ✅ 終了時刻（endTime）: ISO 8601形式（日時情報含む）
- ✅ 終日イベント: 日付のみのタスクはGoogle Calendarの終日イベントとして同期
- ✅ 完了タスク（Done）も含む
- ✅ 削除タスクの検出と同期
- ❌ Notionの「種別」は同期しない
- ❌ 開始時刻なしタスクは同期対象外
- ❌ 開始時刻が範囲外（過去1ヶ月より前、または未来2ヶ月より後）のタスクは同期対象外

### 3. 時刻情報の完全サポート
- Google Calendar APIにより開始時刻・終了時刻を完全にサポート
- Notionで設定した時刻情報がGoogle Calendarに正確に反映されます
- タイムゾーンは`Asia/Tokyo`（日本時間）を使用
...
### 5. 無限ループ防止
- 内容差分チェック（title/status/startTime/endTime）により、変更がない場合はスキップ
- タイムゾーン表記の統一（`+00:00` → `Z`）により、フォーマット差異を吸収
- タイムスタンプは勝者決定にのみ使用
...
| 開始時刻 | 日付.start | start.dateTime / start.date | 時刻指定イベント（dateTime）または終日イベント（date） |
| 終了時刻 | 日付.end | end.dateTime / end.date | 時刻指定イベント（dateTime）または終日イベント（date） |
...
### Phase 1: データ取得
1. Notionから開始時刻が範囲内（過去1ヶ月 ~ 未来2ヶ月）のページを取得
...
### 時刻情報の扱い
- Notionの「日付」プロパティで必ず「時刻を含める」と「終了日」をONにしてください
- **終日イベント**: Notionで時刻を設定せず日付のみ設定すると、Google Calendarで終日イベントとして同期されます
- **時刻指定イベント**: Notionで時刻を設定すると、その時刻でGoogle Calendarに同期されます
- **複数日の終日イベント**: Notionで開始日と終了日を設定（例：12/27-12/28）すると、Google Calendarでも2日間の終日イベントとして同期されます
- タイムゾーンは`Asia/Tokyo`（JST）を使用
- Notionで時刻を設定すると、`time_zone: "Asia/Tokyo"`で保存され、Google Calendarと正しく同期されます
...
4. Notionデータベースに「開始時刻」「終了時刻」プロパティがあるか確認

### タイムゾーンがずれる
1. Notionの「開始時刻」「終了時刻」プロパティで「時刻を含める」がONになっているか確認
2. Notionのアカウント設定でタイムゾーンが「Asia/Tokyo」になっているか確認

### 時刻情報が00:00になる
1. Notionで時刻を明示的に設定してください（例：10:00）
2. 日付のみ設定した場合、00:00 UTCとして扱われます
